...
import { useEnglishChain } from '../hooks/useEnglishChain';
...
interface EnglishChainGameProps {
...
export default function EnglishChainGame({ echo, echoAndWait, mockMode, sdkStatus, onGoHome }: EnglishChainGameProps) {
  const config = GAME_CONFIGS.english;
  const game = useEnglishChain(echo, echoAndWait);
  ...
  const voice = useVoiceInput({ onResult: handleVoiceResult, lang: 'en-US' });
